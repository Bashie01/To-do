...
let input = prompt("Ok, what do you want to do");
const todos = [];
...
while (input !== "quit" && input !== "q") {
  if (input === "list") {
    ...
    const newTodo = prompt("Ok, what is the new todo");
    todos.push(newTodo);
    console.log(`Ok, ${newTodo} has been added to the list`);
  } else if (input === "delete") {
    const index = parseInt(prompt("Ok, to delete please type an index number"));
    if (!Number.isNaN) {
      const deleted = todos.splice(index, 1);
      console.log(`Ok, ${delete [0]} has been deleted`);
    }
  } else {
    console.log("Not a valid index");
  }
  input = prompt("ok, what do you want to do");
}
